...
void SHA256::add(const ByteArray& data)
{
    size_t numBytes = data.size();
    const uint8_t* current = (const uint8_t*) data.data();
...
AString SHA256::operator()(const ByteArray& data)
{
    reset();
    add(data);
    return getHash();
...
AString SHA256::operator()(const AString& text)
{
    reset();
