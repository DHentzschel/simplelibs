...
void MD5::add(const ByteArray& data)
{
    size_t numBytes = data.size();
    const void* tempData = (void*) data.data();
    const uint8_t* current = (const uint8_t*) tempData;
...
AString MD5::operator()(const ByteArray& data)
{
    reset();
    add(data);
    return getHash();
...
AString MD5::operator()(const AString& text)
{
    reset();
    add(text.toByteArray());
    return getHash();
}
