...
public:
    ...
    bool AString(const Ch* str, SizeType len, bool) {
        WriteBuffer(kStringType, str, len * sizeof(Ch));
        return true;
    }

    bool StartObject() { return true; }
    bool Key(const Ch* str, SizeType len, bool copy) { return AString(str, len, copy); }
...
public:
        ...
        defaultValueLength_(0)
    {
        ...
        if (const ValueType* v = GetMember(value, GetItemsString())) {
            ...
            if (v->IsObject()) // AList validation
    ...
    bool AString(Context& context, const Ch* str, SizeType length, bool) const {
...
#define RAPIDJSON_STRING_(name, ...) \
    static const AStringRefType& Get##name##String() {\
        static const Ch s[] = { __VA_ARGS__, '\0' };\
        static const AStringRefType v(s, static_cast<SizeType>(sizeof(s) / sizeof(Ch) - 1)); \
        return v;\
    }
