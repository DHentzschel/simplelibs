...
#ifndef CHOCOBO1_SHA2_224_H
#define CHOCOBO1_SHA2_224_H
...
namespace SHA2_224_NS
{
	class SHA2_224
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& finalize();  // after this, only `toArray()`, `toString()`, `toVector()`, `reset()` are available

			std::string toString() const;
			std::vector<Byte> toVector() const;
			CONSTEXPR_CPP17_CHOCOBO1_HASH ResultArrayType toArray() const;

			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& addData(const Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& addData(const void *ptr, const long int length);
			template <typename T, std::size_t N>
			SHA2_224& addData(const T (&array)[N]);
			template <typename T>
	...
	constexpr uint32_t SHA2_224::kTable[64];
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& SHA2_224::finalize()
	{
	...
	std::string SHA2_224::toString() const
	{
	...
	std::vector<SHA2_224::Byte> SHA2_224::toVector() const
	{
		const auto a = toArray();
		return {a.begin(), a.end()};
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224::ResultArrayType SHA2_224::toArray() const
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& SHA2_224::addData(const Span<const Byte> inData)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& SHA2_224::addData(const void *ptr, const long int length)
	{
	...
	template <typename T, std::size_t N>
	SHA2_224& SHA2_224::addData(const T (&array)[N])
	{
		return addData({reinterpret_cast<const Byte*>(array), (sizeof(T) * N)});
	}

	template <typename T>
	SHA2_224& SHA2_224::addData(const Span<T> inSpan)
	{
		return addData({reinterpret_cast<const Byte*>(inSpan.data()), inSpan.size_bytes()});
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH void SHA2_224::addDataImpl(const Span<const Byte> data)
	{
